// Loads stable Personal OS, then applies schedule polish: 4px gaps, locked AI blocks, exact task details, lighter titles.
// UI patch version: newest-stable-v15
(function(){
  ...
  function parseTime(txt){const m=String(txt||'').match(/(\d{1,2}):(\d{2})\s*-\s*(\d{1,2}):(\d{2})/);return m?{start:+m[1]*60+ +m[2],end:+m[3]*60+ +m[4],raw:m[0]}:null}
  ...
  function isWedDate(date){const d=new Date(String(date||'')+'T00:00:00');return !isNaN(d)&&d.getDay()===3}

  function cleanupState(){
    const st=loadState(); if(!Array.isArray(st.custom))return;
    st.custom.forEach(b=>{
      const title=String(b.title||'').toLowerCase();
      if(/wind down/.test(title)&&b.start)b.end=hm(mins(b.start)+30);
      if(/evening routine/.test(title)&&isWedDate(b.date)){b.start='21:20';b.end='22:30'}
      if(/\bwork\b/.test(title)&&isWedDate(b.date))b.end='21:20';
    });
    saveState(st);
  }

  function css(){
    let s=document.getElementById('personal-os-v15-polish');
    if(!s){s=document.createElement('style');s.id='personal-os-v15-polish';document.head.appendChild(s)}
    s.textContent=`
      ...
      #scheduleView .event.past,#scheduleView .event.dimmed,#scheduleView .event.completed,#scheduleView .event[style*="opacity"]{opacity:.82!important}
      ...
      #scheduleView .event .time{font-weight:800!important;text-align:left!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;max-width:100%!important}
      #scheduleView .event .title{font-weight:700!important;text-align:left!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;max-width:100%!important}
      ...
      .aiDetailFloat{font-size:15.5px!important;line-height:1.35!important;max-width:450px!important}
  ...
  function isAvailable(el){const title=clean((el.querySelector('.title')||{}).textContent||'');return /^available block$/i.test(title)||el.classList.contains('aiFreeHidden')||((el.classList.contains('focus')||el.classList.contains('deep'))&&!el.classList.contains('business')&&!el.classList.contains('personal'))}
  function isGenerated(el){const title=clean((el.querySelector('.title')||{}).textContent||'');return el.classList.contains('aiNeedsDetails')||el.querySelector('.aiDetailShow')||/grouped tasks| pt\. \d+$/i.test(title)||(el.classList.contains('personal')&&!/buy lisa|tok commentary/i.test(title))||(el.classList.contains('business')&&!/work/i.test(title))}
  function dayIsWed(day){const wrap=day.closest('.dayWrap')||day.parentElement||day;return /wed/i.test(wrap.textContent||'')}
  function dayDate(day){try{const days=Array.from(document.querySelectorAll('#scheduleView .day'));const i=days.indexOf(day);if(i>=0&&window.currentMonday&&window.addDays){return ymd(window.addDays(window.currentMonday,i))}}catch(e){}return null}

  function findTaskRecord(el,orig,current,date){
    const st=loadState(); const arr=Array.isArray(st.custom)?st.custom:[];
    const title=clean((el.querySelector('.title')||{}).textContent||el.dataset.stableBaseTitle||'');
    const candidates=arr.filter(c=>Array.isArray(c.taskTexts)&&c.taskTexts.length&&(!date||!c.date||c.date===date));
    function score(c){
      let s=0, ct=clean(c.title||'');
      if(ct===title)s+=40;
      if(title==='Grouped tasks'&&ct==='Grouped tasks')s+=35;
      if(c.start&&orig&&mins(c.start)===orig.start)s+=20;
      if(c.end&&orig&&mins(c.end)===orig.end)s+=20;
      if(c.start&&current&&mins(c.start)===current.start)s+=18;
      if(c.end&&current&&mins(c.end)===current.end)s+=18;
      if(c.start&&orig&&Math.abs(mins(c.start)-orig.start)<=10)s+=8;
      return s;
    }
    let best=null,bestScore=0;
    candidates.forEach(c=>{const sc=score(c);if(sc>bestScore){best=c;bestScore=sc}});
    return bestScore>=35?best:null;
  }

  function setDetailsFromState(el,orig,current,date){
    const rec=findTaskRecord(el,orig,current,date);
    if(rec){
  ...
  function openDetails(el,btn){
    ...
    const raw=(el.dataset.aiTaskTexts||'').trim();
    if(!raw)return;
    ...
    raw.split('||').filter(Boolean).forEach(x=>{const d=document.createElement('div');d.textContent='• '+x;box.appendChild(d)});
    document.body.appendChild(box);
    const r=el.getBoundingClientRect();box.style.left=Math.min(window.innerWidth-460,Math.max(8,r.left+6))+'px';box.style.top=Math.min(window.innerHeight-200,Math.max(8,r.bottom+6))+'px';
    if(btn)btn.textContent='DETAILS';
  }
  function ensureBtn(el){let b=el.querySelector('.aiDetailShow');if(!b){b=document.createElement('button');b.className='aiDetailShow';el.appendChild(b)}b.textContent='DETAILS';b.onclick=e=>{e.preventDefault();e.stopPropagation();e.stopImmediatePropagation&&e.stopImmediatePropagation();openDetails(el,b)};return b}
  function markDetails(el){
    ...
    const need=!!el.dataset.aiTaskTexts&&!protectedType&&!protectedTitle;
  ...
  function normalizeDay(day){
    ...
    rows.forEach(r=>{
      ...
      let top=naturalTop;
      if(prevEnd>0 && Math.abs(start-prevEnd)<=1)top=prevBottom+GAP;
      else top=Math.max(naturalTop,prevBottom+GAP);
      const height=Math.max(12,naturalBottom-top);
      ...
      setDetailsFromState(el,r.t,current,date);
      ...
      el.classList.toggle('aiLockedGenerated',isGenerated(el));
      if(titleEl){titleEl.textContent=title;titleEl.style.fontWeight='700';if(oneLine)titleEl.style.display='none';}
      if(time){time.style.fontWeight='800';time.style.textAlign='left';}
      markDetails(el);
    });
  }
  ...
  cleanupState();css();
  document.addEventListener('click',e=>{const btn=e.target&&e.target.closest&&e.target.closest('#scheduleView .aiDetailShow');if(btn){const el=btn.closest('.event');if(el){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation&&e.stopImmediatePropagation();openDetails(el,btn);return}}const ev=e.target&&e.target.closest&&e.target.closest('#scheduleView .event.aiLockedGenerated');if(ev){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation&&e.stopImmediatePropagation()}},true);
