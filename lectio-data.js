...
(function(){
  ...
  function injectProgressStyles(){
    ...
    t.textContent=`
      #todoView .panel{padding:18px!important;background:linear-gradient(180deg,#070b14,#03050a)!important}.todoLayout{display:grid;grid-template-columns:320px 1fr;gap:20px;align-items:start}.todoSide{border:1px solid #243055;border-radius:18px;background:linear-gradient(180deg,rgba(18,24,48,.92),rgba(7,10,21,.96));padding:16px;position:sticky;top:14px;box-shadow:0 0 0 1px rgba(88,110,255,.08) inset,0 18px 40px rgba(0,0,0,.32)}.todoMain{border:1px solid #17213a;border-radius:18px;background:linear-gradient(180deg,rgba(7,10,19,.88),rgba(3,5,10,.95));padding:16px;box-shadow:0 0 0 1px rgba(143,108,255,.06) inset}.todoSideTitle{font-size:18px;font-weight:1000;margin-bottom:12px;color:#f2efff}.todoDay{margin:12px 0;padding:8px 9px;border-radius:11px;background:rgba(9,13,28,.64);border:1px solid rgba(42,54,92,.72)}.todoDayTop{display:flex;justify-content:space-between;gap:10px;font-size:12px;font-weight:1000;color:#c8bdf0;letter-spacing:.04em}.todoToday{border-color:rgba(73,255,159,.36);background:linear-gradient(180deg,rgba(16,194,119,.09),rgba(9,13,28,.68))}.todoToday .todoDayTop{color:#e8fff4}.todoBar{height:9px;border:1px solid #263455;background:#03050a;border-radius:999px;overflow:hidden;margin-top:6px}.todoBarFill{height:100%;background:linear-gradient(90deg,#49ff9f,#8f6cff);border-radius:999px}.todoChart{margin-top:18px;padding:12px;border:1px solid rgba(40,50,86,.72);border-radius:14px;background:rgba(3,5,10,.44)}.todoChartTitle{font-size:13px;font-weight:1000;color:#fff;margin-bottom:8px}.barChart{height:130px;border-left:1px solid #34415f;border-bottom:1px solid #34415f;display:flex;align-items:end;gap:8px;padding:8px 8px 0 8px;background:linear-gradient(to top,rgba(255,255,255,.04) 1px,transparent 1px) 0 0/100% 25%}.chartCol{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:flex-end;height:100%;min-width:0}.chartBar{width:100%;max-width:22px;border-radius:8px 8px 0 0;background:linear-gradient(180deg,#8f6cff,#26314f);min-height:2px}.chartLabel{font-size:10px;color:#9ca6c5;font-weight:900;margin-top:5px}.todoMain .table{border-color:#1b2540!important}.todoMain .table th:nth-child(1){width:58px}.todoMain .table th:nth-child(2){width:auto}.todoMain .table th:nth-child(3){width:160px}.todoMain .table th:nth-child(4){width:112px}.dragCell{width:34px;color:#64708f;font-weight:1000;cursor:grab;user-select:none;display:inline-block}.todoRow.dragging{opacity:.42}.todoRow.dropAbove{box-shadow:inset 0 2px 0 #8f6cff}.taskPill{display:inline-block;width:auto!important;max-width:min(720px,70vw);min-width:12ch;height:40px;border-radius:999px!important;padding:0 16px!important;background:#050711!important;border-color:#202946!important}.doneRow .taskPill{text-decoration:line-through;color:#777085!important}.todoMain table{table-layout:auto}.todoMain .taskText{white-space:nowrap}.todoMain .panelHead{margin-bottom:14px}@media(max-width:1000px){.todoLayout{grid-template-columns:1fr}.todoSide{position:static}}
    `;
  ...
  function patchTodo(){
    ...
    function normalizeDay(v){if(!v||v==='Later'||v===todayName())return'Today';return v}
    function pct(list){return list.length?Math.round(list.filter(t=>t.done).length/list.length*100):0}
    function esc2(s){return String(s||'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]))}
    function ensureDates(){const now=new Date().toISOString();tasks.forEach(t=>{t.day=normalizeDay(t.day);if(!t.createdAt)t.createdAt=now;if(t.done&&!t.completedAt)t.completedAt=now;if(!t.done&&t.completedAt)delete t.completedAt})}
    function dayLabel(d){return d==='Today'?'Today ('+todayName().slice(0,3)+')':d}
    function dayHtml(days){return days.map(d=>{const list=tasks.filter(t=>normalizeDay(t.day)===d),p=pct(list);return `<div class="todoDay ${d==='Today'?'todoToday':''}"><div class="todoDayTop"><span>${dayLabel(d)}</span><span>${p}%</span></div><div class="todoBar"><div class="todoBarFill" style="width:${p}%"></div></div></div>`}).join('')}
    function weeklyHtml(days){return days.map(d=>{const p=pct(tasks.filter(t=>normalizeDay(t.day)===d));return `<div class="chartCol"><div class="chartBar" style="height:${Math.max(2,p)}%"></div><div class="chartLabel">${d==='Today'?'Today':d.slice(0,3)}</div></div>`}).join('')}
